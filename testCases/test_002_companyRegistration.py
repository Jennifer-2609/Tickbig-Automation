...
# user defined function to generate random email (python)
# def random_generator(size=8, chars=string.ascii_lowercase + string.digits):
#     return ''.join(random.choice(chars) for x in range(size))

    @pytest.mark.sanity
    def test_negativecompanyRegistration(self,setup):
        self.logger.info("********** Test_002_CompanyRegistration Negative Test 1 **********")
        self.driver=setup
        self.driver.get(self.BaseURL)
        self.driver.maximize_window()
        self.l=Login(self.driver)
        ...
        # if result == "User with the same Email / Mobile number already verified.":
        #     assert True
        #     self.logger.info("********** Negative test 1 passed *********")
        # else:
        #     assert False
        #     self.logger.info("********** Negative test 1 failed *********")

        # toast_msg = self.driver.find_elements(By.XPATH, "/html/body/div[1]/div[1]/div/div/div[1]")
        # result = toast_msg[0].text
        # print(result)
        #
        # if result == "User with the same Email / Mobile number already verified.":
        ...
        try:
            ...
            toast_msg = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located((By.XPATH,"/html/body/div[1]/div[1]/div/div/div[1]"))
            )
            result = toast_msg.text
            print(result)
        ...
        self.driver.close()
        self.logger.info("********** Ending Company Registration Negative Test 1 *********")
    ...
    def test_Signupnegative4Home(self, setup):  # already existing user email using signup
        ...
        result = error_msg1[0].text
        if result == "User with the same Email already verified.":
           self.logger.info("Error Message Is: %s", result)
           assert True
    ...
    def test_Signupnegative5Home(self, setup):  # Signup second page(Firstname,Lastname,Mobile) is required
        ...
        error_msg1 = self.driver.find_elements(By.XPATH,
                                               "/html/body/div[1]/div[2]/section/main/div/div/section[2]/section[3]/section/main/form/div[1]/p")
        result = error_msg1[0].text
        if result == "Company Name should not be more than 100 characters.":
            self.logger.info("Error Message Is: %s", result)
            assert True
        ...
        error_msg2 = self.driver.find_elements(By.XPATH,
                                               "/html/body/div[1]/div[2]/section/main/div/div/section[2]/section[3]/section/main/form/div[2]/p")
        result = error_msg2[0].text
        if result == "Brand Name should not be more than 100 characters.":
            self.logger.info("Error Message Is: %s", result)
            assert True
